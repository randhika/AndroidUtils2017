...
public class PersistentCookieStore implements CookieStore {
    ...
    private static final String COOKIE_PREFS = "OkVolleyCookiePrefsFile";

    private static final String COOKIE_NAME_PREFIX = "okvolley_cookie_";

    private final HashMap<String, ConcurrentHashMap<String, HttpCookie>> cookies;

    private final SharedPreferences cookiePrefs;

    private ArrayList<String> onlyAcceptUriStringList;
    ...
    public void add(URI uri, HttpCookie cookie) {
        ...
        if (onlyAcceptUriStringList != null && onlyAcceptUriStringList.size() > 0) {
            for (String uriString : onlyAcceptUriStringList) {
                if (uriString.contains(uri.getPath())) {
                    ...
                    if (BuildConfig.DEBUG) {
                        if (cookies.get(cookie.getDomain()).values() != null) {
                            Collection collection = cookies.get(cookie.getDomain()).values();
                            Iterator iterator = collection.iterator();
                            while (iterator.hasNext()) {
                                HttpCookie cookie1 = (HttpCookie) iterator.next();
                                Log.i(LOG_TAG, "add uri is:" + uri +
                                        ", cookie key is:" + cookie1.getName() +
                                        ", cookie value is:" + cookie1.getValue());
                            }
                        }
                    }
                    // Save cookie into persistent store
                    SharedPreferences.Editor prefsWriter = cookiePrefs.edit();
                    prefsWriter.putString(cookie.getDomain(),
                            TextUtils.join(",", cookies.get(cookie.getDomain()).keySet()));
                    prefsWriter.putString(COOKIE_NAME_PREFIX + cookie.getName(),
            ...
            if (BuildConfig.DEBUG) {
                if (cookies.size() > 0 && cookies.get(cookie.getDomain()).values() != null) {
                    Collection collection = cookies.get(cookie.getDomain()).values();
                    Iterator iterator = collection.iterator();
                    while (iterator.hasNext()) {
                        HttpCookie cookie1 = (HttpCookie) iterator.next();
    ...
    public List<HttpCookie> get(URI uri) {
        ...
        for (String key : cookies.keySet()) {
            if (uri.getHost().contains(key)) {
                if (BuildConfig.DEBUG) {
                    if (cookies.get(key).values() != null) {
                        Collection collection = cookies.get(key).values();
                        Iterator iterator = collection.iterator();
                        while (iterator.hasNext()) {
                            HttpCookie cookie1 = (HttpCookie) iterator.next();
                            Log.i(LOG_TAG, "get uri is:" + uri +
                                    ", cookie key is:" + cookie1.getName() +
                                    ", cookie value is:" + cookie1.getValue());
                        }
                    }
    ...
    public void setOnlyAcceptUriStringList(ArrayList<String> onlyAcceptUriStringList) {
        this.onlyAcceptUriStringList = onlyAcceptUriStringList;
    }
